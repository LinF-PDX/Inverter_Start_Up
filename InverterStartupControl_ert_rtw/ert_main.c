...
void rt_OneStep(void)
{
   ...
   */
  InverterStartupControl_SetEventsForThisBaseStep(eventFlags);
  __enable_irq();
  currentTime = (extmodeSimulationTime_T)
    InverterStartupControl_M->Timing.taskTime0;
  InverterStartupControl_step0();
     ...
     case 1 :
      currentTime = (extmodeSimulationTime_T)
        (((InverterStartupControl_M->Timing.clockTick1+
           InverterStartupControl_M->Timing.clockTickH1* 4294967296.0)) * 2.0);
      InverterStartupControl_step1();
...
int main(void)
{
  ...
  errorCode = extmodeInit(InverterStartupControl_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(InverterStartupControl_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(InverterStartupControl_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
